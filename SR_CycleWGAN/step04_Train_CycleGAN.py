...
if __name__ == '__main__':
    ...
    alpha = 0.01
    AVE_LOSS_D = tools.EXPMA(alpha)
    AVE_REAL_D = tools.EXPMA(alpha)
    AVE_FAKE_D = tools.EXPMA(alpha)
    AVE_MMSE_H = tools.EXPMA(alpha)
    AVE_MMSE_R = tools.EXPMA(alpha)
    ...
    for epoch in range(B_EPOCHS, N_EPOCHS):
        ...
        for minibatch_id in range(minibatch_count):
            ...
            output_real_D = D(IHR)
            output_fake_D = D(ISR)
            loss_real = BCE(output_real_D, torch.ones_like(output_real_D, dtype=torch.float32, device=device))
            loss_fake = BCE(output_fake_D, torch.zeros_like(output_fake_D, dtype=torch.float32, device=device))
            ...
            loss_G_D = BCE(output_fake_G_D, torch.ones_like(output_real_D, dtype=torch.float32, device=device))
            loss = loss_G_D + loss_recon_mmse
            loss.backward()
            optimizerGu.step()  # Update Gu parameters
            optimizerGd.step()  # Update Gd parameters

            V_AVE_LOSS_D = AVE_LOSS_D.expma(loss_D.item())
            V_AVE_REAL_D = AVE_REAL_D.expma(output_real_D.mean().item())
            V_AVE_FAKE_D = AVE_FAKE_D.expma(output_fake_D.mean().item())
            V_AVE_MMSE_H = AVE_MMSE_H.expma(loss_optim_mmse.mean().item())
            V_AVE_MMSE_R = AVE_MMSE_R.expma(loss_recon_mmse.mean().item())

            message = "Epoch:%3d, MinibatchID:%5d/%05d, LOSS:% 6.12f, REAL: % 6.12f, FAKE: % 6.12f, LMSE: % 6.12f, HMSE: % 6.12f" % (
            epoch, minibatch_id, minibatch_count, V_AVE_LOSS_D, V_AVE_REAL_D, V_AVE_FAKE_D, V_AVE_MMSE_R, V_AVE_MMSE_H)
            print(message)

            istep = minibatch_count * (epoch - B_EPOCHS) + minibatch_id
            writer.add_scalar("AVE_LOSS_D", V_AVE_LOSS_D, istep)
            writer.add_scalar("AVE_REAL_D", V_AVE_REAL_D, istep)
            writer.add_scalar("AVE_FAKE_D", V_AVE_FAKE_D, istep)
            writer.add_scalar("AVE_MMSE_R", V_AVE_MMSE_R, istep)
            writer.add_scalar("AVE_MMSE_H", V_AVE_MMSE_H, istep)
