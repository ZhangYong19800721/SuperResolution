...
if __name__ == '__main__':
    ...
    isLoadPretrainedGu, isLoadPretrainedGd = args.isLoadPretrainedGu, args.isLoadPretrainedGd
    ...
    for epoch in range(B_EPOCHS, N_EPOCHS):
        ...
        for minibatch_id in range(minibatch_count):
            ...
            loss = loss_recon_mmse + loss_optim_mmse
            loss.backward()
            optimizerGu.step()  # Update Gu parameters
            optimizerGd.step()  # Update Gd parameters

            V_AVE_LOSS = AVE_LOSS.expma(loss.mean().item())
            V_AVE_LMSE = AVE_LMSE.expma(loss_recon_mmse.mean().item())
            V_AVE_HMSE = AVE_HMSE.expma(loss_optim_mmse.mean().item())

            message = "Epoch:%3d, MinibatchID:%5d/%05d, LOSS:% 6.12f, LMSE: % 6.12f, HMSE: % 6.12f" % (
                epoch, minibatch_id, minibatch_count, V_AVE_LOSS, V_AVE_LMSE, V_AVE_HMSE)
            print(message)

            istep = minibatch_count * (epoch - B_EPOCHS) + minibatch_id

            writer.add_scalar("AVE_LOSS", V_AVE_LOSS, istep)
