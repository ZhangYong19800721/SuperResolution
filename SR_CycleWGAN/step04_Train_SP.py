...
if __name__ == '__main__':
    ...
    learn_rate = 0.0002  # set the learning rate
    ...
    if isLoadPretrainedGu:
        ...
        modelGu_file = open("./model/model_Gu_CPU_005.pkl", "rb")  # open the model file
    ...
    if isLoadPretrainedGd:
        ##########################################################################
        ## load the pretrained G model
        modelGd_file = open("./model/model_Gd_CPU_005.pkl", "rb")  # open the model file
    ...
    if isLoadPretrainedD:
        ...
        modelD_file = open("model/model_D_SP_CPU_005.pkl", "rb")  # open the model file
    ...
    for epoch in range(B_EPOCHS, N_EPOCHS):
        ...
        for minibatch_id in range(minibatch_count):
            ...
            message = "Epoch: %5d, MinibatchID: %5d/%05d, DIFF:% 12.10f[s=% 12.10f], REAL: % 12.10f[s=% 12.10f], FAKE: % 12.10f[s=% 12.10f], MMSE: % 12.10f[s=% 12.10f]" % (
                epoch, minibatch_id, minibatch_count,
                V_AVE_DIFF_L, V_AVE_DIFF_S, V_AVE_REAL_L, V_AVE_REAL_S, V_AVE_FAKE_L, V_AVE_FAKE_S, V_AVE_MMSE_L, V_AVE_MMSE_S
            )
            print(message)
